...
def budget():
    ...
    child7.title("Enter No. of Days and Budget")
    myimg = ImageTk.PhotoImage(Image.open("Days_and_Budget.png"))
    mylabel = Label(child7, image=myimg)
    mylabel.pack()
    daylb=Label(child7,text=" No of   Days:",bg="#F8D7FD")
    daylb.config(font=('Mermaid Swash Caps','11'))
    daylb.pack()
    daylb.place(x=50, y=100)
    days=Entry(child7,bg="#F8D7FD")
    days.config(font=('bold'))
    days.pack()
    days.place(x=165, y=100)
    budlb=Label(child7,text=" Budget:",bg="#F8D7FD")
    budlb.config(font=('Mermaid Swash Caps','11'))
    budlb.pack()
    budlb.place(x=50, y=140)
    bud=Entry(child7,bg="#F8D7FD")
    bud.config(font=('bold'))
    bud.pack()
    bud.place(x=165, y=140)
    submit=Button(child7,text="Submit",command=details,bg="#ab05e8", pady=10, padx=20,fg='white')
    submit.config(font=('LEMON MILK','11','bold'))
    submit.pack()
    submit.place(x=160,y=190)
    child7.mainloop()
...
def changebudget():
    ...
    bud.pack()
    bud.place(x=165, y=140)
    submit = Button(child9, text="Submit", command=lambda: calcpre(bud.get()), bg="#ab05e8", pady=10, padx=20, fg='white')
    submit.config(font=('LEMON MILK', '11','bold'))
    submit.pack()
    submit.place(x=160, y=190)
    child9.mainloop()
...
def store():
    ...
    cur = conn.cursor()
    cur.execute("INSERT INTO category(user_id,list,title,price,Days) VALUES(\"{id}\",\"{list}\",\"{title}\",\"{price}\",\"{NOD}\");".format(id=user_id,list=listToStr,title=abt.get(),price=price, NOD=days.get()))
    conn.commit()
    result()
...
def result2():
    ...
    result.title("Result")
    a=0
    myimg=PhotoImage(file='Result.png')
    #myimg = ImageTk.PhotoImage(Image.open("Result.png"))
    mylabel = Label(result, image=myimg)
    mylabel.place(x=0,y=0)
    for x in range(0,11):
        if mycategory1[x]=="1":
            catlabel=Label(result,text=cats[x])
            catlabel.config(font=('Mermaid','11'))
            catlabel.grid(sticky="W",row=a,column=0)
            pricelabel=Label(result,text=price[x])
            pricelabel.config(font=('Mermaid','11'))
    ...
    totalans.config(font=('Mermaid','11'))
...
def calc(mybudget):
    ...
    child8.title("Budget Window")
    myimg = ImageTk.PhotoImage(Image.open("addmore.jpg"))
    ...
    if CheckVar1.get()==1:
        ...
        one=25000*(no_of_days1)
        two=100000*(no_of_days1)
        three=300000*(no_of_days1)
        five=500000*(no_of_days1)
        if hotel1.get()=="Delux":
            temp=one
        ...
        price[0]=str(temp)
        totalvalue=totalvalue+temp   #tv=tv+price
    print(remaining)
    if CheckVar2.get()==1:
        if photo1.get()=="Basic":
    ...
    if CheckVar11.get()==1:
        ...
        totalvalue = totalvalue + temp
        print(remaining)
    price[11] = str(totalvalue)   #final=(mubudget-remaining)
    ...
    if remaining<0:
        ...
        rem.config(font=('Mermaid','10'))
        rem.pack()
        rem.place(x=120,y=20)
        #do you want to increase budget or change your decisions?
        incbtn=Button(child8,text="Increase Budget",command=changebudget, pady=10, padx=20,bg="#ab05e8",fg='white')
        incbtn.config(font=('LEMON MILK','11','bold'))
        incbtn.pack()
        incbtn.place(x=100,y=100)
        chgbtn=Button(child8,text="Change Decisions",command=details, pady=10, padx=20,bg="#ab05e8",fg='white')
        chgbtn.config(font=('LEMON MILK','11','bold'))
        chgbtn.pack()
        chgbtn.place(x=95,y=170)
    if remaining==0:
        print("Perfect! Budget Exhausted.")
        mystr="Perfect! Budget Exhausted."
        mylbl=Label(child8,text=mystr,bg="#EFA5FD")
        mylbl.config(font=('Mermaid','10'))
        mylbl.pack()
        mylbl.place(x=120,y=20)
        result=Button(child8,text="See Result",command=store, pady=10, padx=20,bg="#ab05e8",fg='white')
        result.config(font=('LEMON MILK','11','bold'))
        result.pack()
        result.place(x=135,y=100)
        mm=Button(child8,text="Main Menu",command=dashboard, pady=10, padx=20,bg="#ab05e8",fg='white')
        mm.config(font=('LEMON MILK','11','bold'))
        mm.pack()
        mm.place(x=134,y=170)
        #result()
    if remaining>0:
        ...
        addbtn=Button(child8,text="Add more things!",command=details2, pady=10, padx=20,bg="#ab05e8",fg='white')
        addbtn.config(font=('LEMON MILK','11','bold'))
        addbtn.pack()
        addbtn.place(x=100,y=80)
        result = Button(child8, text="See Result", command=store, pady=10, padx=20,bg="#ab05e8",fg='white')   #TODO DESIGN
        result.config(font=('LEMON MILK','11','bold'))
        result.pack()
        result.place(x=128,y=150)
        mm = Button(child8, text="Main Menu", command=dashboard, pady=10, padx=20,bg="#ab05e8",fg='white')
        mm.config(font=('LEMON MILK','11','bold'))
        mm.pack()
        mm.place(x=126,y=220)
...
def details():
    ...
    mybudget=bud.get()
    child6=Toplevel()
    child6.geometry("450x500")
    child6.title("Make Choices of Events & Ceremonies")
    ...
    but=Button(child6,text="Submit",pady=10,padx=30,bg="#ab05e8",command=lambda:calc(mybudget),fg='white')
    but.grid(row=15,column=0)
    but.config(font=('LEMON MILK','11','bold'))
    child6.mainloop()
...
def hotelm():
    if CheckVar1.get() == 1:
        ...
        drop.config(font=('Cambria','9','bold'))
...
def bridalm():
    if CheckVar6.get() == 1:
        ...
        drop = OptionMenu(child6, bridal1, "Classic Lehenga", "Diamond Lehenga", "Gown", "Saree") # 2.5l 5l 1l 1.5l
        drop.config(font=('Cambria', '9', 'bold'))
...
def eventm():
    ...
    if CheckVar11.get() == 1:
        ...
        eve1.config(font=('Cambria', '9', 'bold'))
        ...
        eve2.config(font=('Cambria', '9', 'bold'))
        eve3.grid(sticky="W",row=13,column=1)
        eve3.config(font=('Cambria', '9', 'bold'))
        eve4.grid(sticky="W",row=10,column=1)
        eve4.config(font=('Cambria', '9', 'bold'))
...
def addevent():
    child4.withdraw()
    cur=conn.cursor()
    cur.execute("INSERT INTO Wedding(user_id,Date,Title) VALUES(\"{id}\",\"{date}\",\"{title}\");".format(id=user_id,date=cal.get(),title=abt.get()))
    conn.commit()
    budget()
...
def add():
    ...
    child4.title("Wedding Date and Title")
    # child4.configure(bg="#EFA5FD")
    myimg = ImageTk.PhotoImage(Image.open("Choose_date_and_title.png"))
    ...
    a.config(font=('Mermaid Swash Caps','11'))
    a.pack()
    a.place(x=50,y=100)

    cal = DateEntry(child4, width=12, background='#ab05e8',foreground='white', borderwidth=2)
    cal.config(font=('bold'))
    cal.pack()
    cal.place(x=160,y=100)
    abtlb=Label(child4,text=" Title of event:",bg="#EFA5FD")
    abtlb.config(font=('Mermaid Swash Caps', '11'))
    abtlb.pack()
    abtlb.place(x=50,y=150)
    abt=Entry(child4)
    abt.config(font=('bold'))
    abt.pack()
    abt.place(x=160,y=150)
    eventsub=Button(child4,text="Submit",command=addevent, pady=10, padx=30,bg="#ab05e8",fg='white')
    eventsub.config(font=('LEMON MILK', '11','bold'))
    eventsub.pack()
...
def Mywedding():
    ...
    #mycatreplica.config(font=('Mermaid','12'))
...
def check():
    ...
    global var
    child5=Toplevel()
    child5.geometry("600x580")
    child5.title("My Weddings")
    child5.configure(bg="#EFA5FD")
    cur=conn.cursor()
    cur.execute("SELECT id,Date,Title FROM Wedding WHERE user_id=\"{userid}\"".format(userid=user_id))
    mylist=cur.fetchall()
    print(mylist)
    var = StringVar(child5,0)
    for x in mylist:
        myradio=Radiobutton(child5, text=x[2], variable=var, value=x[2],bg="#EFA5FD")
        myradio.config(font=('Mermaid','13'))
        myradio.pack(anchor=W)

    bttn=Button(child5,text="ok",command=Mywedding, pady=10, padx=30,bg="#ab05e8",fg='white')
    bttn.config(font=('LEMON MILK','11','bold'))
    bttn.pack()
...
def dashboard():
    ...
    child3.title("Dashboard")
    myimg = ImageTk.PhotoImage(Image.open("Dashboard.png"))
    mylabel = Label(child3, image=myimg)
    mylabel.pack()
    addbtn=Button(child3,text="ADD A Wedding",command=add,padx="20",pady="20",bg="#ab05e8",fg='white')
    addbtn.config(font=('LEMON MILK', '11', 'bold'))
    addbtn.pack()
    addbtn.place(x=100, y=300)
    chkbtn=Button(child3,text="Check My Weddings",command=check,padx="20",pady="20",bg="#ab05e8",fg='white')
    chkbtn.pack()
    chkbtn.config(font=('LEMON MILK', '11', 'bold'))
...
def login():
    ...
    child2.title("Log in")
    myimg = ImageTk.PhotoImage(Image.open("Login.png"))
    mylabel = Label(child2, image=myimg)
    mylabel.pack()
    nlabel = Label(child2, text=" Name:",bg="#F8D7FD")
    nlabel.config(font=('Mermaid Swash Caps','11'))
    nlabel.pack()
    ...
    plabel.config(font=('Mermaid Swash Caps','11'))
    plabel.pack()
    plabel.place(x=80, y=140)
    password2 = Entry(child2, width=30,show="*",bg="#F8D7FD")
    password2.pack()
    password2.place(x=160, y=140)
    submit = Button(child2, text="Login", command=loggedin, pady=10, padx=30,bg="#ab05e8",fg='white')
    submit.config(font=('LEMON MILK','11','bold'))
    submit.pack()
...
def signup():
    ...
    elabel.config(font=('Mermaid Swash Caps', '11'))
    elabel.pack()
    ...
    plabel.config(font=('Mermaid Swash Caps', '11'))
    plabel.pack()
    plabel.place(x=55,y=190)
    password = Entry(child1,show="*",width=30,bg="#F8D7FD")
    password.pack()
    password.place(x=150,y=190)
    submit=Button(child1,text="Submit",command=fillup,pady=10,padx=20,bg="#ab05e8",fg='white')
    submit.config(font=('LEMON MILK','11','bold'))
    submit.pack()
...
root=Tk()
root.geometry("600x580")
root.title("Welcome to Wedding Management System")
myimg= ImageTk.PhotoImage(Image.open("Welcome.png"))
mylabel= Label(root,image=myimg)
mylabel.pack()
logbtn=Button(root,text="Login",command=login,padx="50",pady="20",bg="#ab05e8",fg='white')
logbtn.config(font=('LEMON MILK','11','bold'))
logbtn.pack()
logbtn.place(x=125,y=300)
subtn=Button(root,text="Sign-Up",command=signup,padx="50",pady="20",bg="#ab05e8",fg='white')
subtn.config(font=('LEMON MILK','11','bold'))
subtn.pack()
